...
class RiscVPipelineSim(RiscVSingleCycleSim):
    ...
    def make_id_ex(self, if_id: PipeReg) -> PipeReg:
        ...
        return PipeReg(
            ...
            predict_taken=self.predict_taken(dec),
        )
    ...
    def step(self, stop_on_self_loop: bool = True) -> PipelineCycleInfo:
        ...
        # WB: commit the value from the previous MEM/WB register first.
        ...
        # MEM: loads and stores use the data port.
        next_mem_wb = PipeReg()
        if self.ex_mem.valid and self.ex_mem.dec:
            ...
            next_mem_wb = PipeReg(
                ...
                predict_taken=self.ex_mem.predict_taken,
            )
        ...
        # EX: ALU, branch, and mispredict detection.
        next_ex_mem = PipeReg()
        mispredict = False
        ...
        if self.id_ex.valid and self.id_ex.dec:
            ...
            if dec.name in M_ALU_NAMES:
                ...
                ex_take_branch = self.branch_taken_values(dec, a, b)
            ...
            if mdu_stall:
                ...
                if ex_take_branch != self.id_ex.predict_taken:
                    mispredict = True
                    if ex_take_branch:
                        mispredict_target = u32(self.id_ex.pc + dec.imm)
                    else:
                        mispredict_target = u32(self.id_ex.pc + 4)

            if stop_on_self_loop and dec.name == "jal" and u32(self.id_ex.pc + dec.imm) == self.id_ex.pc:
        ...
        # ID prediction: determine if current ID instruction predicts taken.
        id_pred_taken = False
        id_pred_target = u32(self.pc + 4)
        if self.if_id.valid and self.if_id.dec:
            id_pred_taken = self.predict_taken(self.if_id.dec)
            if id_pred_taken:
        ...
        if mispredict:
            ...
            next_if_id = PipeReg()  # bubble: target still being fetched
            next_pc = id_pred_target
            flush = False
            stall_flag = False
        elif stall:
            next_id_ex = PipeReg()
